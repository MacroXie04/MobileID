...
class BarcodeCreateSerializer(serializers.Serializer):
    source_type = serializers.ChoiceField(
        choices=[("barcode", "Barcode"), ("session", "Session")]
    )
    ...
    def create(self, validated_data):
        ...
        if src_type == "barcode":
            ...
            if Barcode.objects.filter(barcode=code_to_check, user=user).exists():
                raise serializers.ValidationError(
                    {"input_value": "This barcode already exists."}
                )
            ...
            if not SELENIUM_ENABLED:
                raise serializers.ValidationError(
                    {"source_type": "This feature is currently disabled."}
                )
            ...
            if not code:
                raise serializers.ValidationError(
                    {"input_value": "Failed to retrieve barcode from session."}
                )
        ...
        UserBarcodeSettings.objects.update_or_create(
            user=user, defaults={"barcode": barcode_obj}
        )
