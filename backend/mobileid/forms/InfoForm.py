...
class UserBarcodeSettingsForm(forms.ModelForm):
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        for name, field in self.fields.items():
            base_cls = "form-select" if isinstance(field.widget, forms.Select) else "form-control"
            field.widget.attrs["class"] = f"{field.widget.attrs.get('class', '')} {base_cls}".strip()
    ...
    def clean(self):
        ...
        if pull_enabled in (False, "False", "false"):
            if not selected_barcode:
                self.add_error("barcode", "Please select a barcode when Barcode Pull is disabled.")
        else:
