    ...
    <div class="card p-4 shadow-sm" style="max-width: 500px; width: 100%;">
      ...
      <form @submit.prevent="handleSubmit" novalidate>
        ...
        <div class="mb-3">
          ...
          <div v-if="errors.input_value" class="invalid-feedback">{{ errors.input_value[0] }}</div>
        </div>
      ...
      <div v-if="barcodes.length > 0">
        <ul class="list-group">
          <li v-for="bc in barcodes" :key="bc.id"
            ...
            <span>{{ bc.barcode_type }}: ...{{ bc.barcode.slice(-4) }}</span>
...
const isDeleting = ref(null); // Store the ID of the barcode being deleted
...
const handleSubmit = async () => {
  ...
  try {
    ...
    form.input_value = ''; // 清空输入框
    await fetchBarcodes(); // 重新获取列表
  } catch (err) {
