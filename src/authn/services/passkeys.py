...
def build_registration_options(user: User, exclude_existing: bool = True) -> dict:
    ...
    if exclude_existing:
        ...
        if existing:
            exclude.append(
                PublicKeyCredentialDescriptor(id=base64url_to_bytes(existing.credential_id), type="public-key")
            )
    ...
    if 'user' in result and isinstance(result['user'], dict):
        ...
        if 'id' in result['user'] and isinstance(result['user']['id'], (bytes, bytearray)):
...
def verify_and_create_passkey(user: User, credential: dict, expected_challenge) -> PasskeyCredential:
...
def verify_authentication(credential: dict, expected_challenge) -> User:
    ...
    stored = PasskeyCredential.objects.filter(credential_id=raw_id_b64u).select_related("user").first()
    if not stored:
