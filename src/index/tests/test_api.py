...
class BarcodeDashboardAPITest(APITestCase):
    ...
    def setUp(self):
        ...
        UserProfile.objects.create(
            user=self.school_user, name="School User", information_id="SCHOOL123"
        )
    ...
    def test_dashboard_delete_barcode(self):
        ...
        barcode = Barcode.objects.create(
            user=self.school_user, barcode="deleteme123456789", barcode_type="Others"
        )
    ...
    def test_dashboard_patch_update_share_and_daily_limit(self):
        ...
        self._authenticate_user(self.school_user)
        barcode = Barcode.objects.create(
            user=self.school_user, barcode="patchbar12345678", barcode_type="Others"
        )
    ...
    def test_dashboard_patch_invalid_daily_limit(self):
        ...
        self._authenticate_user(self.school_user)
        barcode = Barcode.objects.create(
            user=self.school_user, barcode="patchbarinvalid", barcode_type="Others"
        )

        url = reverse("index:api_barcode_dashboard")
        # Negative number
        response = self.client.patch(
            url, {"barcode_id": barcode.id, "daily_usage_limit": -1}, format="json"
        )
    ...
    def test_dashboard_post_update_pull_settings(self):
        ...
        url = reverse("index:api_barcode_dashboard")
        data = {"pull_settings": {"pull_setting": "Enable", "gender_setting": "Male"}}
    ...
    def test_dashboard_get_field_states_barcode_disabled_when_pull_enabled(self):
        ...
        # Enable pull setting
        UserBarcodePullSettings.objects.create(
            user=self.school_user, pull_setting="Enable", gender_setting="Female"
        )
    ...
    def test_dashboard_get_field_states_barcode_enabled_when_pull_disabled(self):
        ...
        UserBarcodePullSettings.objects.create(
            user=self.school_user, pull_setting="Disable", gender_setting="Unknow"
        )
...
class ActiveProfileAPITest(APITestCase):
    ...
    def test_school_user_with_linked_profile(self):
        ...
        bc = Barcode.objects.create(
            user=self.school_user, barcode="activeprof123456", barcode_type="Others"
        )
        UserBarcodeSettings.objects.create(
            user=self.school_user, barcode=bc, associate_user_profile_with_barcode=True
        )
    ...
    def test_school_user_with_avatar_adds_data_uri(self):
        self._auth(self.school_user)
        bc = Barcode.objects.create(
            user=self.school_user, barcode="activeprofavatar", barcode_type="Others"
        )
        UserBarcodeSettings.objects.create(
            user=self.school_user, barcode=bc, associate_user_profile_with_barcode=True
        )
...
class TransferCatCardAPITest(APITestCase):
    ...
    def test_warning_for_missing_required_cookie_returns_400(self, mock_process):
        mock_process.return_value = SimpleNamespace(
            header_value="Cookie: a=b", warnings=["Missing required cookie session"]
        )
        url = reverse("index:api_catcard_transfer")
