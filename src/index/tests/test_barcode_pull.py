...
class BarcodePullTest(TestCase):
    ...
    def test_user_not_school(self):
        ...
        # Create a regular user with pull enabled (though UI might prevent this, backend should handle it)  # noqa: E501
        ...
        UserBarcodePullSettings.objects.create(
            user=regular_user, pull_setting="Enable", gender_setting="Male"
        )
        UserBarcodeSettings.objects.create(user=regular_user)

        # generate_barcode for 'User' group forces Identification barcode, ignoring pull settings.  # noqa: E501
