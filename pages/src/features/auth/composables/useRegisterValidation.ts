...
export function useRegisterValidation() {
   ...
   */
  function clearError(field: keyof RegisterFormData | 'general' | string) {
    delete errors[field];
    if (field !== 'general') {
   ...
   */
  function validateField(field: keyof RegisterFormData, formData: RegisterFormData) {
    clearError(field);
   ...
   */
  function validateForm(formData: RegisterFormData) {
    // Clear previous errors
    Object.keys(errors).forEach((key) => delete errors[key]);

    // Validate each field
    const fields: (keyof RegisterFormData)[] = ['username', 'name', 'password1', 'password2'];
   ...
   */
  function setServerErrors(serverErrors: Record<string, string | string[]>) {
    if (serverErrors) {
