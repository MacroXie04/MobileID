...
export function useImageCropper(options: any = {}) {
  ...
  const cropperPreview = ref<HTMLElement | null>(null);
   ...
   */
  async function initializeCropper(imageUrl: string) {
   ...
   */
  async function applyCrop({ maxBase64Length = 30000 }: { maxBase64Length?: number } = {}) {
    ...
    try {
      ...
      // Convert to base64
      let base64String = await new Promise<string>((resolve) => {
      ...
      if (base64String.length > maxBase64Length) {
        ...
        if (!smallerBlob) throw new Error('Failed to create compressed blob from canvas');
