...
import { useAuthenticatedRequest, userInfo } from '@auth';
...
const globalProfile = ref<UserProfile>(cachedInfo?.profile || { name: '', information_id: '' });
...
export function useUserInfo() {
  ...
  async function getUserInfoWithAutoRefresh() {
    try {
      return await apiCallWithAutoRefresh<AuthUser>(`${baseURL}/authn/user_info/`, {
        method: 'GET',
      });
    } catch (error) {
