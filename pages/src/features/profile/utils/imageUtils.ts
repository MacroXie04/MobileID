...
export function validateImageFile(
  ...
  options: { allowedTypes?: RegExp; maxSizeMB?: number } = {}
) {
  const { allowedTypes = /^image\/(jpe?g|png|gif|webp)$/i, maxSizeMB = 5 } = options;
 ...
 */
export async function fileToBase64(file: Blob, removePrefix = true): Promise<string> {
 ...
 */
export function createImageObjectURL(file: Blob, previousUrl: string | null = null) {
  if (previousUrl) {
