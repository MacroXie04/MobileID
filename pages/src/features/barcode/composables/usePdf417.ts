import { ref } from 'vue';
import type { Pdf417Global } from '@barcode/types/pdf417';

const BCMATH_SRC = '/js/bcmath-min.js';
const PDF417_SRC = '/js/pdf417-min.js';

let pdf417LoadPromise: Promise<Pdf417Global> | null = null;

function isScriptReady(globalName: 'PDF417' | 'libbcmath') {
  return typeof window !== 'undefined' && typeof window[globalName] !== 'undefined';
}

function loadScript(src: string, globalName: 'PDF417' | 'libbcmath') {
...
export function usePdf417() {
   ...
   */
  async function drawPdf417(
    canvas: HTMLCanvasElement | null,
    text: string,
    options: { moduleWidth?: number; moduleHeight?: number } = {}
  ) {
    if (!canvas) {
