 ...
 */
async function checkServerHealth(timeoutMs = HEALTH_CHECK_TIMEOUT_MS): Promise<boolean> {
 ...
 */
async function pollUntilReady(shouldRefresh = false): Promise<boolean> {
 ...
 */
async function retryHealthCheck(): Promise<boolean> {
  stopPolling();
  errorMessage.value = '';
  isWakingUp.value = true;
  startElapsedTimer();
