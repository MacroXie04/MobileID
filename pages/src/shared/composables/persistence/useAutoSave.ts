...
export function useAutoSave(
  saveFunction: () => Promise<AutoSaveResult | void>,
  options: {
    debounceMs?: number;
    successToastDuration?: number;
    errorToastDuration?: number;
    lastSavedDuration?: number;
  } = {}
) {
  const {
    debounceMs = 1500,
  ...
  const lastSavedTimer = ref<ReturnType<typeof setTimeout> | null>(null);
   ...
   */
  function showToast(type: AutoSaveStatusType, message: string) {
