...
export function handleAvatarError(
  event: Event,
  options: { profileName?: string; placeholderSrc?: string; onShowInitials?: () => void } = {}
) {
 ...
 */
export function formatDisplayName(name?: string | null, maxLength = 30) {
  if (!name) return 'User';
 ...
 */
export function sanitizeProfile(profile: { name?: string; information_id?: string } | null) {
